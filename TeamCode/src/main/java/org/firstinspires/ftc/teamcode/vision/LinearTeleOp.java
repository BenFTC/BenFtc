...
//------------------------------Wobble------------------------------------------------------------\\
            ...
            if (gamepad1.dpad_left && timeSinceLastPress.milliseconds() > 1000 ) {
                timeSinceLastPress.reset();
                if (armState == 0) {
                    armState = 1;
                    WobbleArm.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
            ...
            if(armState == 1)
            {
                ...
                double WobbleHold = 500;
                double acceptableError = 20;
                double WobbleTick = WobbleArm.getCurrentPosition();

                if      (WobbleTick > (WobbleHold + acceptableError))
                {
                    WobbleArm.setPower(-.25);
                }
                else if (WobbleTick < (WobbleHold - acceptableError))
                {
                    WobbleArm.setPower(.25);
                }
                else
